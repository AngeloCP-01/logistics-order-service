...
describe("Order HTTP (integration)", () => {
  ...
  it("hides another customer's order behind 404 and lets admin read it", async () => {
    fx.stubAddresses.set(ADDR.id, ADDR);
    const created = await request(fx.baseUrl).post("/orders").set("authorization", `Bearer ${fx.signUserJwt(CUSTOMER, "customer")}`).send(body);
    const id = created.body.id;
    expect((await request(fx.baseUrl).get(`/orders/${id}`).set("authorization", `Bearer ${fx.signUserJwt("33333333-3333-7333-8333-333333333333", "customer")}`)).status).toBe(404);
    expect((await request(fx.baseUrl).get(`/orders/${id}`).set("authorization", `Bearer ${fx.signUserJwt("44444444-4444-7444-8444-444444444444", "admin")}`)).status).toBe(200);
  });

  it("does NOT shadow /orders/me with /orders/:id (mount-order regression)", async () => {
    fx.stubAddresses.set(ADDR.id, ADDR);
    await request(fx.baseUrl).post("/orders").set("authorization", `Bearer ${fx.signUserJwt(CUSTOMER, "customer")}`).send(body);
    const res = await request(fx.baseUrl).get("/orders/me").set("authorization", `Bearer ${fx.signUserJwt(CUSTOMER, "customer")}`);
  ...
  it("forbids a customer from listing all orders (admin-only)", async () => {
    const res = await request(fx.baseUrl).get("/orders").set("authorization", `Bearer ${fx.signUserJwt(CUSTOMER, "customer")}`);
    expect(res.status).toBe(403);
  });
});
